...
const isdibingo = () => {
  ...
  const results = (points) => {
    const player = { name: askName, points };
    ...
    if (
      rankingPlayers[0].points > points &&
      rankingPlayers[1].points < points
    ) {
    ...
    if (
      rankingPlayers[1].points > points &&
      rankingPlayers[2].points < points
    ) {
    ...
    rankingPlayers.forEach((player) => {
      nombres.push(player.name), puntos.push(player.points);
    });
  ...
  const rankingPoints = () => {
    let points = 0;
    if (linea) points += 50;
    if (countTurn === 15) points += 9999;
    if (countTurn > 15 && countTurn < 30) points += 1000;
    if (countTurn > 30 && countTurn < 50) points += 500;
    if (countTurn > 50 && countTurn < 80) points += 250;
    if (countTurn > 80 && countTurn < 95) points += 100;
    if (countTurn > 95) points += 50;
    results(points);
    return points;
  };
  ...
  const startAgain = () => {
    const restart = prompt("Quieres volver a empezar el juego? Y/N");
  ...
  const startBingo = () => {
    ...
    const getNumberPlay = numbersBingo();
