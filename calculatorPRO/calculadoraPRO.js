...
function iniciamosCalculadoraPRO() {
  ...
  const introducirNum = () => {
   ...
   do{
    ...
    datos = Number(prompt("Introduce un número para empezar a usar la CalculadoraPRO."));
    if(isNaN(datos)){
  ...
  const operacionesCalculadora = () => {
    ...
    if (numerosIntroducidos.length > 1) {
      const suma = numerosIntroducidos.reduce((valorInicial, segundoValor) => valorInicial + segundoValor).toFixed(3);
      const resta = numerosIntroducidos.reduce((valorInicial, segundoValor) => valorInicial - segundoValor).toFixed(3);
      const multiplicacion = numerosIntroducidos.reduce((valorInicial, segundoValor) => valorInicial * segundoValor).toFixed(3);
      const division = numerosIntroducidos.reduce((valorInicial, segundoValor) => valorInicial / segundoValor).toFixed(3);
