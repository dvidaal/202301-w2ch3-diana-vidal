...
const questions3 = [
  {
    ...
    },
    {
      ...
      status: 0,
      question:
        "CON LA W. Película del oeste americano.",
    },
    {
      letter: "x",
      answer: "xenofobia",
...
const ISDIPasapalabra = () => {
  ...
  const welcomeUser = () => {
    console.log("Bienvenido a ISDI Pasapalabra. Te vamos a ir lanzando las preguntas por cada letra del abecedario. Si no deseas contestar puedes escribir 'PASALACABRA'. Tienes 130 segundos para completar todo el juego. Si en algun momento quieres terminar el juego solo debes escribir 'END'. Mucha suerte y a jugar!");
  };
  ...
  const completeGame = () => {
    if (isOut === false) {
      console.log("El juego ha terminado el total de letras acertadas es: " + contadorOK);
  ...
  const rankingPlayers = (letters) => {
    let player = {player: askName, letters: contadorOK}

    if (contadorOK > ranking[0].letters && contadorOK > ranking[1].letters && contadorOK > ranking[2].letters) {
    ...
    if (contadorOK < ranking[0].letters && contadorOK < ranking[1].letters && contadorOK < ranking[2].letters) {
      console.log("Has tenido tan poca puntuación que ni apareces en el ranking. Estudia un poquito más para la próxima vez! ;)");
    }

    const playerName = [];
    const lettersOk = [];
    ranking.forEach((player) => {playerName.push(player.player), lettersOk.push(player.letters)});
    console.table({ playerName, lettersOk });
  }

  const endGame = () => {
    if (isOut === true) {
      console.log("El total de letras acertadas ha sido de " + contadorOK + " . Gracias por jugar y hasta pronto.");
    } else {
 ...
 const correctAnswer = (index) => {
  ...
  const checkAnswer = (recivedAnswer, index) => {
    let answer = recivedAnswer.toLowerCase();
