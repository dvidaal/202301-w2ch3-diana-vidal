...
const ISDIairlinesPRO = () => {
    ...
    function userCheck() {
        ...
        if (!userName) {
            ...
            alert("Bienvenidx, " + userName + "! " + " A continuación te vamos a mostrar toda la información de nuestros vuelos. Los vuelos con escalas se mostrarán en pantalla con un punto rojo");
        }
    ...
    function userAdminCheck() {
        const whatAreYou = prompt("Ahora debes decirnos si eres 'user' o 'admin'. Escribe una de las dos cosas");
        if (whatAreYou === null) {
    ...
    function searchCostUser() {
        const introducePrecio = +prompt("Introduce un precio para poder mostrar vuelos iguales a ese precio o más baratos.");
        if (!introducePrecio) {
        ...
        if (introducePrecio < 80) {
            ...
            for (let j = 0; j < cheaperFlight.length; j++) {
                console.log("El vuelo con origen: " + (cheaperFlight[j].from) + " y destino: " + (cheaperFlight[j].to) + " tiene un coste de " + (cheaperFlight[j].cost) + " y no realiza ninguna escala.")

            }

            let endUser = prompt("Quieres introducir más precios para seguir viendo vuelos? SI/NO");
            if (endUser === null) {
                endProcess();
            ...
            } else if (endUser.toUpperCase() !== 'SI' || endUser.toUpperCase() !== 'NO') {
...
//adminAction();
    ...
    function generalInfo() {
        console.log("*************TE MOSTRAMOS LOS VUELOS ACTUALES. TUS VUELOS CREADOS SE IRÁN AÑADIENDO A LA LISTA HASTA UN MÁXIMO DE 15 VUELOS ***************")
        flights.forEach(function (vuelo) {
            if (vuelo.scale === true) {
                console.log("El vuelo con origen: " + (vuelo.from) + " y destino: " + (vuelo.to) + " tiene un coste de " + (vuelo.cost) + " y no realiza ninguna escala.");
            } else {
                console.log("El vuelo con origen: " + (vuelo.from) + " y destino: " + (vuelo.to) + " tiene un coste de " + (vuelo.cost) + " y tiene escala. 🔴 ");
            }
        });
    }
    ...
    function createAdmin() {
        if (flights.length >= 15) {
            ...
            let exceedFlight = prompt("Quieres eliminar vuelos para poder crear más? SI/NO");
            ...
            generalInfo();
            alert("Se ha creado tu vuelo con exito. Decide otra vez entre 'CREATE' o 'DELETE.");
            return adminAction();
    ...
    function listWithID(item) {
        ...
        item.forEach((flights) => {
            ...
            console.log(`${flights.id}. ${flights.to}. ${flights.from}. ${flights.scale}`);
        });

    }
    function deleteAdmin() {

        const deleteFlight = +prompt("Introduce el ID del vuelo que desees eliminar.");
